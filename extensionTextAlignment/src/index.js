...
export const App = ({sdk}) => {
  const [value, setValue] = useState(sdk.field.getValue() || 'center');
  ...
  if (hasNoValue) {
    sdk.field.setValue('center');
  }

  const options = [
    { id: 'left', icon: <MdFormatAlignLeft size="20px" /> },
    { id: 'center', icon: <MdFormatAlignCenter size="20px" /> },
    { id: 'right', icon: <MdFormatAlignRight size="20px" /> },
  ]
      ...
      <div className="wrapper">
        {options.map((option) => (
          <div key={option.id}>
            ...
            <label htmlFor={option.id} className={option.id}>{option.icon}</label>
          </div>
        ))}
      </div>
    </div>
