...
function PrivacyPolicy() {
	const { allContentfulLegalText, allContentfulSiteSettings } = useStaticQuery(
		graphql`
			query {
				...
				allContentfulLegalText(
					filter: { internalName: { nin: "PLACEHOLDER Legal Text" } }
				) {
					edges {
						node {
							internalName
	...
	const legalText =
		allContentfulLegalText &&
		allContentfulLegalText.edges.filter(
			(edge) => !edge.node.internalName.includes("PLACEHOLDER")
		)[0].node

	return (
		<Fragment>
			<SEO title="Legal" description="Privacy Policy" siteSettings={site} />
			<ThemeSelector>
				<SplashHeader />
				<ContentWrap>
